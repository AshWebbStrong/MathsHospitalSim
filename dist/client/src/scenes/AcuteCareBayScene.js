...
exports.AcuteCareBayScene = void 0;
// src/scenes/AcuteCareBayScene.ts
const phaser_1 = __importDefault(require("phaser"));
const createPatientActionMenu_1 = require("../utils/createPatientActionMenu");
class AcuteCareBayScene extends phaser_1.default.Scene {
    constructor(room) {
        super("AcuteCareBayScene");
        this.patientSprites = {};
        this.active = false;
        this.room = room; // store the Colyseus room
    }
    create() {
        console.log("[AcuteCareBayScene] ⚡ create()");
        this.room.send("update_location", { location: "acute_care" }); // notify server
        this.active = true;
        this.syncPatients();
        this.room.onStateChange(() => this.syncPatients());
        this.events.once(phaser_1.default.Scenes.Events.SHUTDOWN, () => {
            var _a;
            console.log("[AcuteCareBayScene] 🛑 SHUTDOWN");
            this.active = false;
    ...
    syncPatients() {
        ...
        const patients = Array.from(this.room.state.hospital.patients.values())
            .filter(p => p.location === "acute_care"); // only acute care
        ...
        patients.forEach((p, i) => {
            ...
            const radius = 20;
            console.log(`[AcuteCareBayScene] creating sprite for ${p.id} (${p.name})`);
            const gfx = this.add.graphics().fillStyle(0x44aa44, 1).fillCircle(0, 0, radius);
...
exports.AcuteCareBayScene = AcuteCareBayScene;
