...
exports.OperatingTheatreScene = void 0;
// src/scenes/OperatingTheatreScene.ts
const phaser_1 = __importDefault(require("phaser"));
const createPatientActionMenu_1 = require("../utils/createPatientActionMenu");
class OperatingTheatreScene extends phaser_1.default.Scene {
    constructor(room) {
        super("OperatingTheatreScene");
        this.patientSprites = {};
        this.active = false;
        this.room = room;
    }
    create() {
        console.log("[OperatingTheatreScene] ⚡ create()");
        this.room.send("update_location", { location: "operating_theatre" });
        this.active = true;
        this.syncPatients();
        this.room.onStateChange(() => this.syncPatients());
        this.events.once(phaser_1.default.Scenes.Events.SHUTDOWN, () => {
            var _a;
            console.log("[OperatingTheatreScene] 🛑 SHUTDOWN");
            this.active = false;
    ...
    syncPatients() {
        ...
        const patients = Array.from(this.room.state.hospital.patients.values())
            .filter(p => p.location === "operating_theatre");
        ...
        patients.forEach((p, i) => {
            ...
            const radius = 20;
            console.log(`[OperatingTheatreScene] creating sprite for ${p.id} (${p.name})`);
            const gfx = this.add.graphics().fillStyle(0xffaa00, 1).fillCircle(0, 0, radius);
...
exports.OperatingTheatreScene = OperatingTheatreScene;
